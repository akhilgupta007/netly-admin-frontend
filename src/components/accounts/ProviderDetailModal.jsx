...
import { X, CreditCard, Key, ShieldAlert, ShieldCheck, Award, Star } from "lucide-react";
import { toast } from "react-toastify";
import { getInitials } from "@/lib/utils";

export default function ProviderDetailModal({ provider, isOpen, onClose, onSuspendBanTrigger, onReactivateTrigger }) {
      ...
      <div className="relative bg-white rounded-3xl max-w-xl w-full p-4 shadow-2xl z-10 border border-secondary-bg animate-scale-up mx-10 max-h-[95vh] flex flex-col font-onest">
        ...
        <div className="space-y-4 overflow-y-auto pr-2 flex-1 scrollbar-thin">
          ...
          <div className="space-y-1.5">
            <span className="text-[10px] text-text-muted block">Services Offered</span>
            <div className="flex flex-wrap gap-1.5">
              {["Deep Clean", "Move-Out", "Post-Construction"].map((s, idx) => (
                <span key={idx} className="bg-blue-50 text-blue-500 text-[10px] px-2.5 py-1 rounded-full">
                  {s}
                </span>
              ))}
            </div>
            ...
            <div className="space-y-3">
              {providerQuestions.map((q, idx) => (
                <div key={idx} className="text-xs space-y-0.5">
                  <span className="text-[9px] text-text-muted block">Question {q.num}</span>
                  <strong className="text-text-primary font-normal block">{q.text}</strong>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-1.5 border border-secondary-bg rounded-2xl p-3">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              <span className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 border border-blue-200 text-[10px] font-semibold px-2 py-0.5 rounded-full">
                <Star size={10} className="shrink-0 fill-current" /> Founding Provider
              </span>
            </div>
          </div>
