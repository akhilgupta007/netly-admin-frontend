    ...
    <div className="flex items-center justify-between border-t border-border-main px-5 py-3 bg-white select-none rounded-b-3xl">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="w-7 h-7 flex items-center justify-center border border-border-main rounded-lg hover:bg-page-bg transition disabled:opacity-50 text-[10px] font-bold cursor-pointer"
        >
          &larr;
        </button>
        {Array.from({ length: totalPages }).map((_, idx) => (
          <button
            ...
            className={`w-7 h-7 flex items-center justify-center border rounded-lg transition text-[10px] font-semibold cursor-pointer ${currentPage === idx + 1
