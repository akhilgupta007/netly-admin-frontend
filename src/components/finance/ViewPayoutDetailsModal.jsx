  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl p-4 max-w-xl w-full space-y-4 shadow-xl relative animate-scale-up max-h-[90vh] overflow-y-auto scrollbar-thin font-onest">
        <div className="flex items-center justify-between gap-4 border-b border-secondary-bg pb-4">
          <div className="flex items-center gap-3">
            <div className="w-7 h-7 rounded-lg bg-primary-bg-muted text-white flex items-center justify-center font-light text-xs select-none uppercase shrink-0">
              {payout.initials}
            </div>
          ...
          <div className="flex items-center gap-3 h-full">
            <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold ${
              payout.status === "Completed" ? "bg-[#E8F8F5] text-[#10B981]" :
        ...
        <div className="space-y-2">
          ...
          <div className="border border-secondary-bg rounded-2xl overflow-hidden bg-white">
            <table className="min-w-full divide-y divide-secondary-bg text-left text-xs">
              <thead className="bg-secondary-bg text-text-primary text-[10px] font-semibold">
                <tr>
              ...
              <tbody className="divide-y divide-secondary-bg text-text-primary">
                {bookings.map((bk) => (
            ...
            <div className="bg-[#F7F9FA] px-4 py-2.5 flex justify-end items-center gap-4 text-xs border-t border-secondary-bg">
        ...
        <div className="space-y-3 pt-2">
          ...
          <div className="space-y-4 relative pl-2">
            <div className="absolute left-[23px] top-4 bottom-4 w-0.5 bg-secondary-bg" />
            {mockHistory.map((hist, idx) => (
              <div key={idx} className="flex items-center justify-between gap-4 relative">
                <div className="flex items-center gap-3">
                  {hist.type === "success" && (
                    <div className="w-8 h-8 rounded-full bg-green-100 text-green-500 flex items-center justify-center shrink-0 z-10 border border-white">
                      <Check size={14} className="stroke-3" />
                    </div>
                  )}
                  {hist.type === "fail" && (
                    <div className="w-8 h-8 rounded-full bg-red-50 text-red-500 flex items-center justify-center shrink-0 z-10 border border-white">
                      <X size={14} className="stroke-3" />
                    </div>
                  )}
                  {hist.type === "skip" && (
                    <div className="w-8 h-8 rounded-full bg-amber-100 text-amber-500 flex items-center justify-center shrink-0 z-10 border border-white">
                      <AlertCircle size={14} className="stroke-3" />
                    </div>
                  )}
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-text-primary">{hist.date}</span>
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${
                        hist.type === "success" ? "bg-[#E8F8F5] text-[#10B981]" :
